...
//---------------------------------------
$offset = ($page - 1) * $limit;
$query = 'SELECT id,student_operation_id,exam_date,exam_date_text,type_id,type_name,category,result FROM v_school_exams ';
if ($id != -1) {
    $query .= ' where id = '.$id;
} else {
...
$query = 'SELECT count(*) AS Total FROM v_school_exams';
if ($id != 0) {
    $query .= ' where id = '.$id;
} else {
    $query .= ' where student_operation_id = '.$student_operation_id;
