...
//---------------------------------------
$offset = ($page - 1) * $limit;
$query = 'SELECT id,exam_group_id,student_operation_id,student_number,number,student_full_name,school_unit_id,school_unit_name_short,theory_exam_id,theory_result_id,theory_result,theory_motive_id,polygon_exam_id,polygon_result_id,polygon_result,polygon_motive_id,city_exam_id,city_result_id,city_result,city_motive_id,cert_id,cert_number FROM v_exam_gibdd_group';
if ($id != -1){
	$query .= ' where id = '.$id;
} else {
	$query .= ' where exam_group_id = '.$exam_group_id.' and active = 1 order by number, student_full_name';
}
...
\'polygon_exam_id\':' . intval($arr['polygon_exam_id']) . ',
\'polygon_result_id\':' . intval($arr['polygon_result_id']) . ',
\'polygon_result\': \'' . escape4js($arr['polygon_result']) . '\',
\'polygon_motive_id\':' . intval($arr['polygon_motive_id']) . ',
...
$query = 'SELECT count(*) AS Total FROM v_exam_gibdd_group';
if ($id != -1){
	$query .= ' where id = '.$id;
} else {
	$query .= ' where exam_group_id = '.$exam_group_id.' and active = 1';
}
