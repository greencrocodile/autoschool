...
Ext.onReady(function () {
    ...
    if (checkUserRole('EGS_R')) {
        ...
        //**********variables**********
        var selectedSchoolExamGroupId = -1;
        // var newExamGroupSchoolId = -1;
        // var selectedExamGroupSchoolStudentId = -1;
        // var newExamGroupSchoolStudentId = -1;
        // var selectedLearningGroupId = -1;


        //**********models**********
       

        // Ext.define('ExamSchoolStudentsResults', {
        ...
        // Ext.define('ExamSchoolGibddRegStaffResults', {
            // extend: 'Ext.data.Model',
            // proxy: {
                // type: 'jsonp',
        ...
        // Ext.define('ExamSchoolUnitResults', {
            // extend: 'Ext.data.Model',
            // proxy: {
                // type: 'jsonp',
		...
		schoolExamGroupsStore.getProxy().extraParams = {id: -1, school_unit_id: (sessvars.userPrivileges.indexOf('#EGS_AS#') == -1?sessvars.schoolUnitId:-1)};
        schoolExamGroupsStore.load();

        //**********grids**********
        var schoolExamGroupsGrid = Ext.create('Ext.grid.Panel', {
			region: 'center',
            store: schoolExamGroupsStore,
            margin: '10 0 10 0',
            ...
            listeners: {
                itemclick: function (view, record) {
                    if (record) {
                        selectedSchoolExamGroupId = record.data.id;
                    }
                },
                itemdblclick: function (view, record) {
                    if (record) {
						selectedSchoolExamGroupId = record.data.id;
						if (checkUserRole('EGS_R')){
							editSchoolExamGroup(record.data.id, 0, function(){
								schoolExamGroupsStore.reload();
							});
						}
                    }
                }
        ...
        var panelEGS = Ext.create('Ext.panel.Panel', {
            region: 'north',
            border: false,
            bodyCls: 'alt-background',
            items: [
                {
                    xtype: 'label',
                    html: '<span style="font-size: 180%; font-weight: bold">&nbsp;Школьные экзамены&nbsp;</span>'
                }
            ]
        });

        var editSchoolExamGroupPanel = Ext.create('Ext.panel.Panel', {
			region: 'north',
            border: false,
            bodyCls: 'alt-background',
            items: [
                    ...
                    listeners: {
                        click: function () {
							addGroup(function(id){
									editSchoolExamGroup(id,1,function(){
											schoolExamGroupsStore.reload();
										}
									)
								}	
                ...
                },
                {
                    ...
                    listeners: {
                        click: function () {
							deleteSchoolExamGroup(selectedSchoolExamGroupId);
                        }
                    }
                },
				{
					...
					listeners: {
						click: function () {
							if (selectedSchoolExamGroupId != -1){
								exportDocuments(-1,-1,-1,null,-1,-1,selectedSchoolExamGroupId,-1,-1,'school_exam_documents', function () {})
							}
                        }
					}
                }
        ...
        var schoolExamGroupsListPanel = Ext.create('Ext.panel.Panel', {
			layout: 'border',
			region: 'center',
            bodyCls: 'alt-background',
            id: 'schoolExamGroupsListPanel',
            items: [
                editSchoolExamGroupPanel,
                schoolExamGroupsGrid
            ]
        });
