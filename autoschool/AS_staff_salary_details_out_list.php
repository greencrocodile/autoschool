...
//---------------------------------------
$offset = ($page - 1) * $limit;
$query = 'SELECT id,staff_id,salary_month,salary_year,learning_program_id,learning_program_name_short,article_id,article_name,amount,coefficient,value,total FROM v_staff_salary_details_out';
if($id != -1){
	$query .= ' where id = '.$id;
} else {
...
$query = 'SELECT count(*) AS Total FROM v_staff_salary_details_out';
if($id != -1){
	$query .= ' where id = '.$id;
} else {
	$query .= ' where salary_month = ' . $salary_month . ' and salary_year = ' . $salary_year . ' and staff_id = '.$staff_id.' order by id limit '.$limit.' offset '.$offset;
