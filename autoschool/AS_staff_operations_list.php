...
//---------------------------------------
$offset = ($page - 1) * $limit;
$query = 'SELECT id,staff_id,operation_date,operation_date_text,article_id_in,article_name_in,article_id_out,article_name_out,value,comment FROM v_staff_operations ';
if ($staff_id != -1){
...
while ($arr = $result->fetch_assoc()) {
	...
	\'operation_date\': ' . (($arr['operation_date_text'] == '') ? 'null' : 'new Date(\'' . escape4js($arr['operation_date_text']) . '\')') . ',
...
$query = 'SELECT count(*) AS Total FROM v_staff_operations ';
if ($staff_id != -1){
	$query .=  'where staff_id = ' . $staff_id;
	if ($in_article == 1) {
