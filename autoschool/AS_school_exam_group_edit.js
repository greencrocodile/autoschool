function editSchoolExamGroup(id, is_new, callback) {
    //**********variables**********
    var selectedSchoolExamGroupStudentId = -1;
	var selectedSchoolExamGroupStudentOperationId = -1;
    //**********stores**********
    var schoolExamGroupsStore = Ext.create('Ext.data.Store', {
        model: 'SchoolExamGroupsModel',
        proxy: {
            type: 'jsonp',
    ...
    schoolExamGroupsStore.getProxy().extraParams = {id: id};
    schoolExamGroupsStore.load({
        callback: function (records, operation, success) {
            var group = schoolExamGroupsStore.getById(id);
            if (group) {
				...
				var studentsGrid = Ext.create('Ext.grid.Panel', {
					...
					viewConfig : {
						listeners : {
							celldblclick : function(view, cell, cellIndex, record,row, rowIndex, e) {
								...
								if (checkUserRole('EGS_E')){
									if(clickedDataIndex == 'theory_result'){
										if(record.data.theory_exam_id == -1){
											checkStudentAllowExam(record.data.student_operation_id,'school_theory_exam', function(allow){
												if (allow == 1){
													checkStudentSuccessfullExam(record.data.student_operation_id,'school_theory_exam',function(exist){
														if (exist == 1){
															Ext.MessageBox.confirm('Подтверждение', 'Этот студент уже сдал экзамен ранее. Продолжить?', function (btn) {
																if (btn == 'yes') {
																	editSchoolTheoryExam(record.data.theory_exam_id, record.data.student_operation_id, record.data.id, record.data.theory_result_id,record.data.theory_motive_id, function(){
										...
										// editSchoolTheoryExam(record.data.theory_exam_id, record.data.student_operation_id, record.data.id, record.data.theory_result_id,-1, function(){
											// studentsStore.reload();
										// });
									}
									if(clickedDataIndex == 'city_result'){
										if(record.data.city_exam_id == -1){
											checkStudentAllowExam(record.data.student_operation_id,'school_city_exam', function(allow){
												if (allow == 1){
													checkStudentSuccessfullExam(record.data.student_operation_id,'school_city_exam',function(exist){
														if (exist == 1){
															Ext.MessageBox.confirm('Подтверждение', 'Этот студент уже сдал экзамен ранее. Продолжить?', function (btn) {
																if (btn == 'yes') {
																	editSchoolCityExam(record.data.city_exam_id, record.data.student_operation_id, record.data.id, record.data.city_result_id,record.data.city_motive_id, function(){
																		studentsStore.reload();
																	});
																}
															});
										...
										// editSchoolCityExam(record.data.city_exam_id, record.data.student_operation_id, record.data.id, record.data.city_result_id,-1, function(){
											// studentsStore.reload();
										// });
									}
								}
					...
					listeners: {
						itemclick: function (view, record) {
							if(record){
								selectedSchoolExamGroupStudentId = record.data.id;
								selectedSchoolExamGroupStudentOperationId = record.data.student_operation_id;
							}
						}
					}
				})
					...
					items: [
						{
							...
							margin: '5 5 5 5',
							disabled: !checkUserRole('EGS_E'),
							listeners: {
								click: function () {
									addStudent(function(id){
										editSchoolExamStudent(id,1,function(){
												studentsStore.reload();
											}
										)
									});
						...
						},
						{
							...
							listeners: {
								click: function () {
									deleteSchoolExamStudent(selectedSchoolExamGroupStudentId,function(){
										studentsStore.reload();
									})
								}
							}
						...
						},
						{
							...
							listeners: {
								click: function () {
									if (selectedSchoolExamGroupStudentOperationId != -1){
										exportDocuments(-1,-1,-1,null,-1,-1,-1,selectedSchoolExamGroupStudentOperationId,-1,'school_exam_students_documents', function () {})
									}
								}
							}
						}
