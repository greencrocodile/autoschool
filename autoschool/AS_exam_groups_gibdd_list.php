...
//---------------------------------------
$offset = ($page - 1) * $limit;
$query = 'SELECT id,number,learning_group_id,school_unit_id,school_unit_name_full,school_unit_name_short,exam_date,exam_date_text,protocol_number,protocol_date,protocol_date_text,gibdd_reg_staff_id,gibdd_reg_staff_initials_name,gibdd_reg_date,gibdd_reg_date_text FROM v_exam_gibdd_groups ';
if ($id != -1){
	$query .= ' where id = '.$id;
} else {
...
$query = 'SELECT count(*) AS Total FROM v_exam_gibdd_groups';
if ($id != -1){
	$query .= ' where id = '.$id;
} else {
	$query .= ' where active != 2';
